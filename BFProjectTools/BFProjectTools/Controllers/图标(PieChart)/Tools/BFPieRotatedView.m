...
#define K_MAX_SPEED             12.0f  // 最大速度
#define K_POINTER_ANGLE         (M_PI / 2)


@interface BFPieRotatedView ()
...
// pieChart
@property (strong, nonatomic) BFPieRenderView *pieRender;
...
@implementation BFPieRotatedView
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        self.pieRender = [[BFPieRenderView alloc]initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
        self.pieRender.dataSource = self;
        [self.pieRender setStartPieAngle:0];
        [self.pieRender setAnimationSpeed:1.0];
        [self.pieRender setLabelFont:[UIFont fontWithName:@"DBLCDTempBlack" size:15]];
        [self.pieRender setPieCenter:CGPointMake(frame.size.width/2, frame.size.height/2)];
        [self.pieRender setUserInteractionEnabled:NO];
        self.pieRender.delegate = self;
        //        CAGradientLayer *gradient = [CAGradientLayer layer];
        //        gradient.frame = self.bounds;
        //        gradient.colors = [NSArray arrayWithObjects:(id)[UIColor blackColor].CGColor,
        //                           (id)[UIColor grayColor].CGColor,nil];
        //        [self.layer insertSublayer:gradient atIndex:0];
        [self addSubview:self.pieRender];
    }
    return self;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (wedges > [mColorArray count]) {
        NSLog(@"Number of colors is not enough: please add %ld kinds of colors.",wedges - [mColorArray count]);
...
- (void)outPie
{
    [self.pieRender pieSelected:self.selectedIndex];
...
{
    pieChart.isShowTextLayer = NO;
    pieChart.isUserInteractionEnabled = NO;
}

- (void)pieChart:(BFPieRenderView *)pieChart didSelectSliceAtIndex:(NSUInteger)index
{
    pieChart.isShowTextLayer = YES;
    pieChart.isUserInteractionEnabled = YES;
    [pieChart pieSelected:index];
}
