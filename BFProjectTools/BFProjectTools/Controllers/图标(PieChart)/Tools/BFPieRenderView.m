...
@interface BFPieRenderView (Private)
...
@implementation BFPieRenderView
{
...
//        return;
    ...
    // 旋转textLayer文本
    if(_textLayerArray.count != 0 ){
        for (CATextLayer *tempTextLayer in _textLayerArray) {
            double middleAngle = (layer.startAngle + layer.endAngle)/2.0;
            CGAffineTransform transform = CGAffineTransformIdentity;
            transform = CGAffineTransformRotate(transform,  fabs(middleAngle - _startPieAngle)+(-M_PI_2));
            tempTextLayer.affineTransform = transform;
        }
    }
